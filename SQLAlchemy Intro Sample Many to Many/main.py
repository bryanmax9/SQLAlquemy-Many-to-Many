...
def add_section(session: Session):
    ...
    :return:        None
    """
    print("Which course does this section belong to?")
    ...
    while not unique_section:
        ...
        section_count = session.query(Section).filter(Section.course == course, Section.sectionNumber == sectionNumber,
                                                       Section.semester == semester, Section.sectionYear == sectionYear,
                                                       Section.building == building, Section.room == room,
                                                       Section.schedule == schedule, Section.startTime == startTime,
                                                       Section.instructor == instructor).count()
        unique_section = section_count == 0
        if not unique_section:
            print("We already have a section with that information.  Try again.")
...
def add_student_section(sess):
    ...
    """The student object instance is mapped to a specific row in the Student table. But adding
    the new section to its list of sections does not add the new Enrollment instance to this session.
    That Enrollment instance was created and added to the Student's enrollments list inside of the
    add_section method, but we don't have easy access to it from here. And I don't want to have to
    pass sess to the add_section method. So instead, I add the student to the session. You would
    think that would cause an insert, but SQLAlchemy is smart enough to know that this student
    has already been inserted, so the add method takes this to be an update instead, and adds
    the new instance of Enrollment to the session. THEN, when we flush the session, that
    transient instance of Enrollment gets inserted into the database, and is ready to be
    committed later (which happens automatically when we exit the application)."""
    sess.add(student)  # add the Enrollment to the session
    sess.flush()
...
def add_section_student(sess):
    ...
    unique_section_student = False

    while not unique_section_student:
        enrollment_count: int = sess.query(Enrollment).filter(Enrollment.sectionId == section.sectionId,
                                                               Enrollment.studentId == student.studentID).count()
        unique_section_student: bool = enrollment_count == 0
...
def select_section(sess) -> Section:
    """
    Select a section by the combination of department_abbreviation, course_number, section_number, section_year, and semester.
    :param sess:    The connection to the database.
    ...
    while not found:
        ...
        semester = input("Semester--> ")
        section_count: int = sess.query(Section).filter(Section.departmentAbbreviation == department_abbreviation,
        ...
        if not found:
            print("No section found with that combination. Try again.")
    section: Section = sess.query(Section).filter(Section.departmentAbbreviation == department_abbreviation,
                                                  Section.courseNumber == course_number,
                                                  Section.sectionNumber == section_number,
                                                  Section.sectionYear == section_year,
...
def delete_section(session: Session):
    """
    Prompt the user for a section by sectionId and delete it.
    :param session: The connection to the database.
    :return:        None
    """
    ...
    if n_enrollments > 0:
        print(f"Sorry, there are {n_enrollments} students enrolled in that section. Remove them first, "
...
def delete_student_section(session: Session):
    """
    Remove a student from a particular section.
    :param session: The current database session.
    :return:        None
    """
    print("Prompting you for the student and the section they are no longer enrolled in.")
    student: Student = select_student(session)
...
def delete_section_student(session: Session):
    """
    Remove a student from a particular section.
    :param session: The current database session.
    :return:        None
    """
    print("Prompting you for the section and the student who is no longer enrolled in that section.")
    section: Section = select_section(session)
    student: Student = select_student(session)
...
def list_section(session: Session):
    ...
    :return:        None
    """
    sections: [Section] = list(session.query(Section).order_by(Section.departmentAbbreviation, Section.courseNumber, Section.sectionNumber))
...
def list_student_section(session: Session):
    """
    Prompt the user for the student, and then list the sections that the student has enrolled in.
    :param session: The connection to the database
    :return:        None
    """
    student: Student = select_student(session)
    recs = session.query(Student).join(Enrollment, Student.studentID == Enrollment.studentId).join(
        Section, Enrollment.sectionId == Section.sectionId).filter(
        Student.studentID == student.studentID).add_columns(
        Student.lastName, Student.firstName, Section.departmentAbbreviation, Section.courseNumber, Section.sectionNumber, Section.semester, Section.sectionYear).all()
    for stu in recs:
        print(f"Student name: {stu.lastName}, {stu.firstName}, Section: {stu.departmentAbbreviation} {stu.courseNumber}-{stu.sectionNumber} ({stu.semester} {stu.sectionYear})")
...
def list_section_student(session: Session):
    """
    Prompt the user for the section, then list the students who are enrolled in that section.
    :param session: The connection to the database.
    :return:        None
    """
    section: Section = select_section(session)
    recs = session.query(Section).join(Enrollment, Section.sectionId == Enrollment.sectionId).join(
